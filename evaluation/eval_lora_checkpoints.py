...
os.environ['HPGDIFF_LOGDIR'] = str(project_root / 'generated_lora')
os.makedirs(os.environ['HPGDIFF_LOGDIR'], exist_ok=True)


def create_argparser():
    parser = argparse.ArgumentParser()
    parser.add_argument('--base_model_path', type=str, default='../checkpoints/model.pt')
    parser.add_argument('--lora_path', type=str, default='../LoRA_scripts/lora_checkpoints')
...
def resolve_path(path):
    ...
    if not resolved.is_absolute():
        resolved = evaluation_root / resolved
    return resolved


def get_next_log_dir():
    base_dir = os.environ['HPGDIFF_LOGDIR']
    existing_dirs = glob.glob(os.path.join(base_dir, 'eval_lora_log_*'))
    if not existing_dirs:
        return os.path.join(base_dir, 'eval_lora_log_1')
    existing_numbers = [int(d.split('_')[-1]) for d in existing_dirs]
    next_number = max(existing_numbers) + 1
    return os.path.join(base_dir, f'eval_lora_log_{next_number}')
...
def generate_lora_samples(args, num_samples):
    ...
    adapter_flags = f'--model_path {model_path} --lora_path {lora_path}'
    ...
    command_args = f'{model_flags} {diffusion_flags} {adapter_flags} {data_flags} {transformer_flags} {psl_attn_flags}'
    logger.info('Generating LoRA samples...')
...
def evaluate_lora_checkpoint(args):
    ...
    try:
        ...
        logger.info(f"Processing LoRA adapter: {args.lora_path}, test level: {args.test_level}")
        sample_start_time = time.time()
        res = generate_lora_samples(args, num_samples)
        ...
        if not res:
            logger.error(f"LoRA sample generation failed: {args.lora_path}")
            return False
        ...
        logger.error(f"Failed to evaluate LoRA checkpoint: {str(e)}")
        import traceback
...
def main():
    ...
    success = evaluate_lora_checkpoint(args)
