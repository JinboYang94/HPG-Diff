...
def mysolidspy(path):
    ...
    if not(np.allclose(KG.dot(UG)/KG.max(), RHSG/KG.max())):
        logger.info("The system is not in equilibrium!")
...
def fem_compliance_i(topology,dict_array,i, gen_dir):
    ...
    fem_dir = gen_dir + "fem_files/"
    os.makedirs(fem_dir, exist_ok=True)
    nodes_topo = create_files(np.squeeze(topology[i]), BC_conf, load_position, load_x_value, load_y_value, fem_dir)

    UC_in, E_nodes_in, S_nodes_in = mysolidspy(fem_dir)
    comp = np.sum(np.multiply(E_nodes_in, S_nodes_in))
    logger.info(f"analyzing sample {i+1}, comp is: {comp}")
...
def compute_deflection(num_samples, summaries, gen, gen_dir):
    res = np.empty(num_samples)
    for i in range(num_samples):
        res[i] = fem_compliance_i(gen, summaries, i, gen_dir)
    return res
...
def analysis(num_samples, summaries, gen, gen_dir):
    d = compute_deflection(num_samples, summaries, gen, gen_dir)
...
def order_list(num_samples, num_folder):
    ...
    return lst
def re_order_tab(num_samples, tab):
    return tab[:num_samples]
...
def hpgdiff_analysis(num_samples, num_folder, summaries, gen_dir):
    ...
    return analysis(num_samples, summaries, gen, gen_dir)
...
def print_results(tab):
    high_error_ratio = np.count_nonzero(tab[0] > 0.3) / len(tab[0])
    error_median = np.median(tab[0])

    positive_values = tab[0][tab[0] > 0]
    num_positive = len(positive_values)
    total_values = len(tab[0])
    proportion_positive = num_positive / total_values

    negative_values = tab[0][tab[0] <= 0]
    num_negative = len(negative_values)
    total_values = len(tab[0])
    proportion_negative = num_negative / total_values

    logger.info(f"Average CE: {tab[0].mean()}\nMedian CE: {error_median}\nHigh CE: {high_error_ratio}\nAverage VFE: {tab[2].mean()}\nProportion of LD: {tab[1].mean()}\nProportion of FM: {tab[3].mean()}")
    logger.info(f"real Average CE: {abs(tab[0]).mean()}")
    logger.info(f"Proportion of real error: {proportion_positive}\nAverage real error: {abs(positive_values.mean())}")
    logger.info(f"Proportion of improvement: {proportion_negative}\nAverage improvement: {abs(negative_values.mean())}")
